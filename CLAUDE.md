...
**Stack**: React 19 + Vite, React Router v7 (JSX route trees, not config objects), TanStack React Query v5, `@tanstack/react-table` v8.21.3 (headless, used only as `DataTable`'s sort-state/header engine — pinned deliberately, v9 is an unrelated from-scratch rewrite, don't upgrade casually), Supabase (Postgres + Auth + Storage) as the sole backend, SCSS modules, `react-hook-form`, `recharts`, `jspdf`/`jspdf-autotable` for PDF export. `axios` and `js-cookie` are in `package.json` but unused in `src/` — don't reach for them; use `supabase-js` and Supabase's own session persistence instead.
...
**List/CRUD pages follow one recipe** (see `src/pages/user/hr/employeeManagement/` as the reference implementation): a page-level `usePaginatedQuery({ queryKey, queryFn, pageSize, defaultSortBy })` (`src/hooks/usePaginatedQuery.js` — generic URL-searchParams-driven pagination/search/sort/filter hook, mirrors state into the URL so it's shareable/bookmarkable; also exposes an additive multi-column `sorting`/`setSorting` array alongside the legacy single-column `sortBy`/`sortOrder`, serialized via `src/hooks/sortParam.js`, plus optional URL-driven `pageSize`/`setPageSize`) drives the query; a `tableConfig.jsx` in the same folder defines one `columns` array (`key`, `label`, `getValue`, `editable`, `editor`, `options`, `section`, `required`, `show`, plus optional `sortable`/`sortKey` to enable a clickable/shift-clickable sortable header for that column) that is fed to **both** `DataTable` (list view) and `DataSidebar`→`DataForm` (create/edit slide-in panel) — `section` groups fields in the form, `editor` selects the widget in both places via a shared lookup map (`src/components/dataTable/editors/Editors.jsx`: `text`, `number`, `date`, `dateTime`, `select`, `asyncSelect`, `textarea`, `link`, `image`, `drivePicker`), and `show` hides a column from the **create/edit form** (not the table — `DataTable` never reads `show` at all; a page must pre-filter its own `columns` array if it wants a column hidden from the table too) — `show: false` for a static hide, or `show: (formValues, rowData) => boolean` for a field whose relevance depends on another field's live value (note `required` has no function form: conditional requiredness goes through `col.validate(value, {formValues})`, and `DataForm` strips function-hidden fields from the submitted payload). Sibling `filterConfig.js`, `sortConfig.js`, `layoutConfig.js` (and sometimes `overviewConfig.js` for a KPI tab) configure `ActiveFiltersBar`/`SortBar`/view-toggle/`OverviewCards` — `sortConfig.js`/`SortBar` are the legacy single-column-dropdown convention and should be dropped once a page migrates to sortable headers instead (see Employee Management). Delete/save actions go through an `ActionModal` confirmation before calling the mutation hook, including **row-level table edits**: passing `editableRows` to `DataTable` adds a per-row Edit/Save/Cancel actions column (exclusive — only one row editable at a time), using the same field-level validation as the sidebar form via the shared `src/components/crud/dataForm/EditableField.jsx`, and still routing through the page's existing `ActionModal`/mutation hook rather than a separate save path. `DataTable` can also show a per-row flag/badge column — `showCompleteness` (sugar for "any `required: true` column is empty," via `src/features/_shared/getMissingFields.js`) or a page-supplied `getRowFlags(row, columns) => string[]` (+ optional `flagIcon`/`flagTooltipTitle`) for a custom rule (e.g. Payroll Export's "needs reconciliation") — rendered via `src/components/dataTable/RowFlagBadge.jsx`. When building a new CRUD feature page, copy this file set (`tableConfig.jsx`, `filterConfig.js`, `layoutConfig.js`, and `sortConfig.js` only if not using sortable headers) rather than inventing a new shape.

Generic CRUD scaffolding lives in `src/components/crud/` (`PageLayout`, `PageHeader`, `PageActions`, `SortBar`, `ActiveFiltersBar`, `PageResult`, `NoResult`, `OverviewCards`, `DataForm`, `DataForm`'s per-field `EditableField.jsx`) and `src/components/dataTable/` (`DataTable`, `DataTableCell`, `EditableTableRow.jsx` (row-edit mode), `RowFlagBadge.jsx` (the flag/badge column), `tanstackColumnAdapter.js` (translates `tableConfig.jsx` columns into TanStack column defs for sorting), `editors/`) — reuse these rather than building bespoke table/form UI per feature.
