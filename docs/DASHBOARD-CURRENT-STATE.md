...
**Redesign (2026-07, Sales Reports redesign):** headline tiles restructured 4 → 8, following the same "~7 headline numbers"/"what → so what → now what" methodology already applied to Finance Reports, telling doc-02's own Sales Story directly ("Where are deals in the pipeline, will we hit target, and are we fulfilling what we sold?") in two blocks: **Pipeline & Conversion** (CRM-side, forward-looking) — Pipeline Attainment, Pipeline Coverage (new — fills doc-02's previously-unrepresented "pipeline value & coverage ratio" bullet), Win Rate (promoted from a buried Order Book sub-metric to its own tile), Pipeline Velocity (new, synthesized); **Execution, Bookings & Concentration** (SAP-side, backward-looking) — Sales Cycle, Invoice Budget Attainment, Sales Order Book, Client Concentration (new). Also added a Pipeline Stage funnel chart (doc-02 explicitly calls for one; this page previously had zero stage-composition view) and a SAP-only Bookings vs Invoiced Revenue trend chart (orders booked vs. invoices billed, trailing 12 months — distinct from the existing CRM-vs-SAP `realizedVsPipelineData` chart). OTIF and return/credit-memo-rate (also in doc-02's KPI list) remain **blocked** — OTIF needs Production (`OWOR`)/Inventory (`OITW`) extraction, credit-memo rate needs `ORIN`/`RIN1`, neither synced into Supabase yet (same class of gap as Finance's DIO/CCC blocker). A rep/region performance cut is deferred pending verification that `sap_customers`/`clients` region fields are reliably populated — not built speculatively.

**Source-labeling clarity pass (2026-07):** every tile sublabel/sub-metric and chart legend now names its literal source table instead of a generic word — "Pipeline Target" (`sales_targets`) vs. "Revenue Budget" (`sales_budgets`) vs. "Sales Orders" (`sap_sales_orders`) vs. "Invoiced Revenue" (`sap_invoices`) — and the tile formerly named "Customer Concentration" was renamed **"Client Concentration"** to match this page's own "Top Clients" chart and underlying `clients` table (Finance Reports' "Customer" tiles are a different, SAP-sourced table — the two words are deliberately not interchangeable across the app). See `DASHBOARD-CONVENTIONS.md`'s "Source-labeling convention" for the full tag vocabulary.
...
Pipeline Coverage (`activePipelineValue / pipelineTargetRevenue`), Pipeline Velocity (`totalOpportunities × avgDealSize × winRatePct/100 ÷ avgDaysToClose`), Customer Concentration (top-5 `topClientsData` ÷ `pipelineWonRevenue`), and Backlog Gap (`orderBookValue − totalInvoiced`) are all derived client-side in `overviewConfig.js`, not RPC fields.
...
Charts: Realized (SAP) vs Pipeline (CRM) Revenue — explicitly "not blended" · Order Book by Rep (same `order_value_myr` figures as the scorecard, reshaped for the chart) · **Bookings vs Invoiced Revenue** (new, trailing 12 months, SAP-only) · Gross Profit by Rep · **Pipeline Stage** (new — funnel-style, via the existing horizontal-bar renderer, same technique Leads Overview already uses for its own stage chart) · Product-Type Mix · Lead-Source ROI · Top Clients.

**Notable gaps:** no AI Summary, no period-over-period delta (both present elsewhere in the app). `sales_budgets.budget_gross_profit` was proposed but never added to the real migration — no GP-based budget view possible today. `sales_attainment_snapshots` exists but is unused — attainment is always live-recomputed, so it can shift retroactively if invoices are cancelled/corrected after the fact. (The previously-noted stray `console.log(dashboard)` debug line was checked during the 2026-07 redesign and is no longer present — this gap note is now resolved.)
