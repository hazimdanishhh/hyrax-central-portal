...
SAP Business One does ship a native Cash Flow Categories feature (`OCFW`/`OCFT`) — investigated live during this phase and ruled out, not assumed: 6 empty placeholder rows, last real activity 2021, never actually used for real cash-flow tracking. So the statement is built entirely from data already live from Phases 1-2 (Net Profit + D&A add-back, ± change in Working Capital, all Assets/Liabilities/Equity classification reused directly from Phase 2's own structurally-confirmed `level2_ancestor` categories), strengthened by a genuine bonus finding from that same investigation: `OBNK`, a real, currently-maintained (through last month) per-bank-account monthly movement table, gives this statement a second independent reconciliation check beyond the GL's own aggregate cash-balance delta. **One open item, flagged not blocking:** Hyrax's chart of accounts has only one Level-2 node under Liabilities (no long-term-liability category), and neither repo has found evidence of an interest-bearing loan account in it — the statement's classification assumes there isn't one hiding there; worth a finance conversation to confirm. Full derivation, the two ruled-in/ruled-out SAP tables, and the reconciliation-check math: `hyrax-data-platform/docs/sap-data-architecture-plans/06-finance-expansion-execution-plan.md`'s Phase 3 and `get_finance_dashboard_rpc.sql`'s own comments.
