...
**Deferred from the above pass — documented, not yet built:**

1. **The actual salesperson self-service surface.** The RPC's new self-scoping guard is currently unreachable from the frontend (`sales/reports` stays `SAL/MGM` + `manager`-gated) — no staff user can reach it today. Two live options, still undecided: a new dedicated page (mirroring this app's existing My Attendance/Team Attendance split) vs. reusing `sales/reports` for every role with content that branches on `canAccess`. The guard is already the foundation either direction needs, with zero further RPC change.
2. **The cross-module drill-through problem this exposes.** Sales Reports' tiles/charts link into Finance pages (`/app/finance/invoices`, `/app/finance/payments`, both `FIN`-only) via `canAccessInvoices`/`canAccessPayments`. A salesperson viewing their own scoped data has no equivalent Sales-side destination for those links today. Needs either Sales-scoped Invoice/Payment/Bill list pages under the Sales nav, or link-target logic that resolves to whichever version of a page the current viewer can actually reach — the same problem will recur for Finance/Operations/other departments' own Reports pages once their own self-service work starts.
3. **Roll the same identity-aware self-scoping guard pattern out to other department RPCs** — `get_finance_dashboard`, `get_operations_dashboard`, and future ones. Sales Reports is the first application of this pattern, not the only place it belongs.
