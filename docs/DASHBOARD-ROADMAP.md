...
**Filter-propagation fix + Top Products (2026-08).** Audited against three usage goals (department meetings, management meetings, salesperson self-service) and found the page's "Owner" filter — the only person-scoping filter — only ever reached CRM/`sales_leads` data; every SAP-sourced figure (the Invoice Budget Scorecard chief among them) had no filter parameter that could even reach it. Fixed: `get_sales_reports_dashboard` now resolves the CRM owner to a SAP `sales_rep_code` once (via `employee_sales_rep_mapping`) and applies it uniformly across `base_invoices`/`base_orders`/`base_payment_apps`/`budget_math`, so every downstream SAP-sourced field inherits the filter — selecting a salesperson now scopes the whole page, not half of it. Filter relabeled "Owner" → "Salesperson" accordingly, and its dropdown narrowed to Sales-department employees (was every employee company-wide). Also added `topProductsData` — the page's first real "actual sales" product cut, sourced from `sap_invoice_lines` (billed/audited) joined to `sap_items`, since the only prior product cut was `sales_leads.product_type`, a 3-value CRM enum with no link to a real SKU. A `RDR1`-sourced "booked" companion chart is a cheap future addition given the CTE shape is nearly identical, not built this pass. Also added a hand-rolled role/department access guard (mirroring `get_finance_dashboard_rpc.sql`'s own precedent) — closes a real gap where any authenticated caller hitting the RPC directly, bypassing the frontend gate, got full unfiltered company-wide data; a `staff`-role caller is now force-scoped to their own `employees.id` server-side. See `RPC-REFERENCE.md`'s `get_sales_reports_dashboard` section for the full detail.

**O2C funnel restructure (2026-08).** Full design and phased execution plan lives in [`docs/SALES-REPORTS-RESTRUCTURE-PLAN.md`](./SALES-REPORTS-RESTRUCTURE-PLAN.md) — don't duplicate it here, that doc is the source of truth for this work. Summary: Phase 0 (a real fail-open scoping bug found during this work — a Salesperson filter selecting an unmapped rep was silently returning company-wide SAP data instead of zero rows) and Phase 1 (reordering the page from "SAP block then CRM block" into the actual Order-to-Cash narrative — Pipeline → Order → Invoice → Payment — plus a new manager-only "Needs Attention" exception panel and wiring the Rep Funnel Scorecard's already-computed-but-never-rendered Collected leg) are both **built 2026-08**. Phase 2 (a "My Sales" individual self-service page, reusing the RPC's self-scoping guard from the prior pass) is fully designed in that doc but **not yet built**.

**Deferred, tracked in `SALES-REPORTS-RESTRUCTURE-PLAN.md` Part 6 — documented, not yet built:**

1. **The actual salesperson self-service surface (Phase 2 above).**
2. **The cross-module drill-through problem this exposes.** Sales Reports' tiles/charts link into Finance pages (`/app/finance/invoices`, `/app/finance/payments`, both `FIN`-only) via `canAccessInvoices`/`canAccessPayments`. A salesperson viewing their own scoped data has no equivalent Sales-side destination for those links today.
3. **Roll the same identity-aware self-scoping guard pattern out to other department RPCs** — `get_finance_dashboard`, `get_operations_dashboard`, and future ones.
4. **Commission tile, Stalled Deals signal, true Order→Invoice/Invoice→Payment time-in-stage, and Sales-scoped read-only Invoice/Payment list pages** — see `SALES-REPORTS-RESTRUCTURE-PLAN.md` Part 6 for each.
...
9. Whether Finance should get its own read-only Clients/Vendors browse page over `sap_customers` (mirroring the Sales-side SAP Clients page built 2026-08) — **decided 2026-08: not now.** Finance's current `sap_customers`/vendor access (`financeMetadataService.js`'s `searchSapCustomers`/`searchSapVendors`) is a narrow `customer_code`/`customer_name` async-select lookup embedded as a filter chip on Invoices/Payments/Bills/Vendor Payments/Financial Reports — never a full-row browse need — and no one has asked for more than that. A standalone browse page would mostly duplicate that lookup with no analytical depth, and matches the caution already on record in §2.5 against shipping a supplier-master-only stub ahead of real AP data. Revisit if a real need shows up, or naturally alongside the PO-chain-driven Procurement Reports work already tracked in §2.5.
10. ~~Whether the "Last Updated" freshness banner (Sales Reports, Operations Reports, Financial Reports) should show the oldest `last_run_at` across a dashboard's watched pipelines (conservative weakest-link) or the most recent one (optimistic).~~ **Reversed 2026-08** — was deliberately the oldest (see the now-updated `DASHBOARD-CONVENTIONS.md`'s freshness-banner convention), with a past-incident precedent for exactly that conservatism (a banner that looked too fresh because a stale-dependency pipeline was missing from the watched list). The user was shown this tradeoff explicitly and chose the optimistic "most recent" display anyway, accepting that the banner can now look fresher than the true worst-case staleness among watched pipelines. Fixed in `financeMetadataService.js`/`salesReportsMetadataService.js`/`operationsMetadataService.js`/`salesOrdersMetadataService.js` (the last one dead code today, fixed for consistency).
