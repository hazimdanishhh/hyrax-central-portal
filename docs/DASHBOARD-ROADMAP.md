...
**Execution plan (2026-07):** the full phased build-out — Accounts Payable chain, General Ledger, and per-warehouse inventory valuation, plus an early DSO-formula reconciliation and new list pages for the new entities — is tracked in `hyrax-data-platform/docs/sap-data-architecture-plans/06-finance-expansion-execution-plan.md`. That doc owns the SAP table/column/schema decisions and phase sequencing per this repo's usual governance split; update its per-phase "Status" line as work lands, and flip the rows above from blocked to live here as each phase ships.
