...
**Execution plan (2026-07, phase numbering updated 2026-08):** the full phased build-out — Accounts Payable chain (**Phase 1, done 2026-07**), General Ledger (**Phase 2, done 2026-07**, plus the follow-up passes above), a Cash Flow Statement (**Phase 3, added 2026-08, not started** — opens with a scoping spike into SAP B1's native Cash Flow Categories module), and per-warehouse inventory valuation (**Phase 4, renumbered from Phase 3 in 2026-08 to make room for the above, not started**), plus an early DSO-formula reconciliation (**Phase 0, done 2026-07**) and new list pages for the new entities (**done alongside Phase 1, and again as a Phase 2 follow-up**) — is tracked in `hyrax-data-platform/docs/sap-data-architecture-plans/06-finance-expansion-execution-plan.md`. That doc owns the SAP table/column/schema decisions and phase sequencing per this repo's usual governance split; update its per-phase "Status" line as work lands. Phase 2's RPC and `gl_migration.sql` are both deployed — no outstanding manual step remains for Phase 2 as of 2026-07.
...
- Add a dual-labeling disclosure on Leads Overview + Finance Salesperson Health (§5 below) — the CRM-vs-ERP duality (still open; distinct from the orders-vs-invoices duality resolved 2026-07, see §5).
...
**Now (zero data-model work):** don't silently pick a winner for Duality B. Relabel Leads Overview's scorecard as "Pipeline Attainment — CRM, self-reported, vs. quota"; add a shared disclosure that it and the SAP-recognized-revenue views use different identity keys and revenue definitions and may legitimately disagree.
