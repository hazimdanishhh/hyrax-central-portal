...
Remaining to design in (contract-first, even if null-filled until data lands): a net-of-returns caveat/net revenue figure (blocked on Returns/Credit-Memo extraction); **Pass 2 of this same redesign** — a new "Cash Conversion & Liquidity" chart section (DSO/DPO trend line, an approximate Operating Cash Flow waterfall) plus AR/AP aging composition strips, deferred deliberately since the DSO/DPO trend needs a non-trivial new derivation deserving its own focused build; DIO and a finalized Cash Conversion Cycle (blocked on per-warehouse inventory valuation — Phase 3, `dso`/`dpo` are both already built so CCC is one input away). State plainly in the UI going forward: Finance now has a true P&L, balance sheet, and a right-sized headline scorecard; DIO/CCC and the Pass 2 trend/cash-flow charts are the remaining gaps.
