...
- [`docs/setup/WORKSPACE-STATUS-NOTIFICATIONS-DEPLOYMENT-GUIDE.md`](./setup/WORKSPACE-STATUS-NOTIFICATIONS-DEPLOYMENT-GUIDE.md) — deployment steps for the three dynamic multi-recipient Workspace notifications (`task.status_changed`, `project.status_changed`, `document.attached`), the first to use the loop-and-emit-per-recipient pattern instead of `target_payload_keys` alone.
...
- **Self-service attendance rules** (`attendance.clocked_in_remote`, `attendance.autoclockout_approaching`, `attendance.clocked_out_remote`) are now built too, alongside a new Dashboard-home attendance card and sidenav widget — one change-triggered event on clock-in, a two-cutoff scheduled-scan warning ~15 minutes before each of the two real, already-live `auto_clock_out()` cutoffs, and one change-triggered event covering every way a remote session can end (manual, scanner-triggered, or cutoff-triggered — they already converge on the same `UPDATE`, so one trigger covers all three). See `docs/ATTENDANCE-SELF-SERVICE-ARCHITECTURE.md` for the full design, including a scanner-ingestion race-condition guard and an `hours_worked` double-counting fix found and fixed alongside this work.
- **Dynamic multi-recipient events** (`task.status_changed`, `project.status_changed`, `document.attached`) are now built too, for UAT (2026-08) — the first events whose audience is a list resolved at fire time (a task's assignees, a project's members) rather than one `target_payload_keys` uuid or a static role/department. Solved by having the trigger function loop over recipients and call `emit_notification_event()` once per recipient, rather than extending `fan_out_notification_event()` itself — see `docs/PROJECTS-TASKS-ARCHITECTURE.md`'s Notifications section and `docs/setup/WORKSPACE-STATUS-NOTIFICATIONS-DEPLOYMENT-GUIDE.md`. The array-valued-payload-key engine extension named above as a possible alternative was deliberately not built — the per-recipient loop needed no engine change and was just as easy to write per event.
...
- **Digest/batching** is now built (2026-08), in response to real UAT feedback rather than speculatively — `send-queued-emails.ts` groups each tick's pending rows by `to_email` before sending: a recipient with only one pending row gets it unchanged, but 2+ pending rows for the same address go out as a single email (generic subject, each item's own subject/body/link concatenated in order). `email_queue`/`fan_out_notification_event()` are untouched — one row is still written per notification per recipient; only the send step combines them. See `email_log`, which still gets one row per original notification even when several went out in the same physical email.
