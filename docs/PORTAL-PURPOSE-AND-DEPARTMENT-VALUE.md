...
- Journal Entries and Chart of Accounts stopped being dead ends — a GL line's business-partner code now resolves to a real name instead of a raw, unreadable code, and clicking a postable account jumps straight to the GL activity that produced its balance.
...
- _Displayed, but not a real workflow_: Journal Entries is framed as an "audit trail," but only in the narrow sense of a read-only GL mirror — there's no compliance-reporting layer (regulatory reports, control attestations, exception flags) built on top of it.
