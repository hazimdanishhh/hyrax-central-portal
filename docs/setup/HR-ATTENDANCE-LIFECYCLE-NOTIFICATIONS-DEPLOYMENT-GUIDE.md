...
- [ ] Run the updated **`supabase/sql_editor/schedule_check_employee_confirmations_cron.sql`** — creates a new job, `check-employee-lifecycle-daily`, that calls all four `employees`-table functions (due-soon, overdue, status-mismatch, contract-action-due) in one daily tick.
...
**`employee.confirmation_status_mismatch`** (one-shot):

- [ ] Pick a test employee: an active (non-Probation) status, `confirmation_date = null`, `join_date` more than 6 months ago, `confirmation_mismatch_notified_at = null`.
- [ ] Run `select public.check_employee_confirmation_status_mismatches();` — confirm a notification lands for HR only (no manager row), and `confirmation_mismatch_notified_at` is now set.
- [ ] Re-run — confirm it does not fire again (one-shot).

**`employee.contract_action_due`** (one-shot):

- [ ] Pick a test employee: active status, an `employment_type` whose name contains "contract", `end_date` within the next 30 days, `contract_action_reminder_sent_at = null`.
