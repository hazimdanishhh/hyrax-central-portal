...
export const fetchMyAttendanceThisWeek =
  ...
  async ({ weekStartISO, todayISO }) => {
    const { data, error } = await supabase
      ...
      .select(
        "employee_uuid, work_date, day_state, evidence_quality, approval_state, is_weekend, first_in, last_out, hours_worked, overtime_hours, is_early_leave, is_late_arrival, daily_activities, is_on_leave, leave_type_codes, is_leave_attendance_conflict, is_insufficient_half_day_hours, has_leave_fraction_error, is_worked_on_holiday, holiday_hours_worked",
      )
