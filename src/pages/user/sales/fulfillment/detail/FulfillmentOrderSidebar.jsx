...
import "./FulfillmentOrderSidebar.scss";
 ...
 * an experimental parallel sidebar on the Sales Orders page itself (moved
 * out of src/pages/user/sales/orders/detail/, which now only has the
 * original, unmodified SalesOrderSidebar.jsx again). See
 * docs/SALES-ORDER-PIPELINE-ROADMAP.md §2.2.
 *
 * Order Lines/Matched Invoice(s)/Matched Payment(s) are each collapsed by
 * default -- their data only fetches once the viewer actually expands that
 * section (see useSalesOrderLines/useInvoicesForSalesOrder/
 * usePaymentsForSalesOrder's own `enabled` param), and each carries a
 * "View all" button (styled after PayrollReconciliationSidebar.jsx's own
 * precedent) drilling through to the real Invoices/Payments list, filtered
 * to exactly this order's own matched doc_entrys via the `docEntries`
 * filter (invoicesService.js/paymentsService.js).
 */
export default function FulfillmentOrderSidebar({ selectedRow }) {
  const { canAccess } = useAccessControl();

  // Same queryKey shape as FulfillmentTracker.jsx's own fallback
  // useFulfillmentOrder call, so React Query dedupes into a single network
  // request when both fire for the same docEntry (a direct/shared URL case).
  const { data: fulfillmentOrder, isLoading: fulfillmentLoading } =
    useFulfillmentOrder(selectedRow?.doc_entry);
  ...
  const deliveryRecordCount = fulfillmentOrder?.delivery_record_count ?? 0;
  const matchedInvoiceCount = fulfillmentOrder?.matched_invoice_count ?? 0;
  const totalInvoiced = fulfillmentOrder?.total_invoiced_myr ?? 0;
  const totalPaid = fulfillmentOrder?.total_paid_myr ?? 0;
  const totalApplied = fulfillmentOrder?.total_applied_myr ?? 0;
  const totalOutstanding = fulfillmentOrder?.total_outstanding_myr ?? 0;
