 ...
 * RM + count shape as Invoices'/Sales Orders' own configs.
 *
 ...
 * page (it IS the list), so the default would silently 404. "." resolves
 * against the parent `fulfillment` route, so it also dismisses an open
 * :docEntry detail sidebar, which is correct.
 *
 ...
 * deliveryOverdueOnly values used below are the negative-side filters added
 * alongside this file so each tile's drill-through returns exactly the rows
 * the tile counted.
 */
export function getFulfillmentOverviewConfig(kpis) {
