...
4. **Finance's manager-gated pages** (Invoices, Bills, Journal Entries, Chart of Accounts) — fixed by dropping the `manager` restriction, and **`MGM` was dropped from all six of Finance's Tier-1 operational pages** (Invoices, Payments, Bills, Vendor Payments, Journal Entries, Chart of Accounts, Claims Management), not just the four that were manager-gated: the app's own docs call these "Tier-1-only," and Tier 1 is explicitly defined as individual-contributor/daily in `DASHBOARD-CONVENTIONS.md`'s own tier table, so manager-gating (or granting exec-level `MGM`) any of them contradicts the app's declared IA. All six now share one identical gate: `FIN`, no role restriction. **Superseded 2026-09**: this restriction was deliberately reversed, the same way item 2's analogous Sales restriction was reversed — Finance Reports' and Sales Reports' own drill-through KPI tiles/charts, and a Sales Order sidebar's "MATCHED INVOICE(S)"/"MATCHED PAYMENT(S)" cards, all link into these six pages, and an `MGM` viewer needs to actually open the target, not just see a disabled link. `MGM` (company-wide, not manager-gated, matching how `FIN` itself is ungated on these R3 routes) was re-added to route/sidenav/dashboard-card together (`FinanceRoutes.jsx`, `sideNavLinkData.js`, `departmentLinkCardData.js`), plus every drill-through gate computed against these routes (`FinancialReports.jsx`'s `canAccessFinanceOps`, `Reports.jsx`'s `canAccessInvoices`/`canAccessPayments`, and `SalesOrderSidebar.jsx`'s matched-invoice/matched-payment link checks) and the underlying RLS (`supabase/policies/mgm_finance_access_parity_fix.sql`, mirroring `mgm_sales_access_parity_fix.sql`'s own template). Cash Flow/Balance Sheet/Income Statement (already `FIN;MGM` manager-gated) are untouched by this reversal — that's the separate, still-open "KNOWN OPEN DISCREPANCY" noted in `FinanceRoutes.jsx`, not part of this change. **Superseded again, 2026-09 (standardization pass, Finance Phase 1):** Payments and Vendor Payments stopped being standalone routes and became tabs of Invoices and Bills respectively — `/app/finance/invoices` is now a tabbed layout ("Invoices"/"Payments" tabs, at `/app/finance/invoices/list` and `/app/finance/invoices/payments`) and `/app/finance/bills` is now a tabbed layout ("Bills"/"Payments" tabs, at `/app/finance/bills/list` and `/app/finance/bills/vendor-payments`). So what this item counted as six separate Tier-1 operational pages is now four pages — Invoices & A/R, Bills & A/P, Journal Entries, Chart of Accounts — plus Claims Management; the `FIN;MGM` gate itself carried over unchanged onto every tab, nothing about the access decision above changed, only the route/page shape it's applied to. `Reports.jsx`'s `canAccessInvoices`/`canAccessPayments` (Sales Reports' own drill-through flags into these tabs) stay two separate variables — pointing at the two different tab paths (`/app/finance/invoices/list` vs. `/app/finance/invoices/payments`) — but now evaluate an identical `canAccess({departments:["FIN","MGM"]})` check, since both tabs share one gate; they're no longer functionally separate flags, just two names for the same permission check.
...
6. **HR Reports vs. HR's other pages (2026-09)** — the inverse of item 4: unlike Finance, HR's Tier-1 pages (`hr/employees`, `hr/attendance`, `hr/leaves`, `hr/organization-chart`, `hr/onboarding`, `hr/offboarding`) were deliberately **kept `HR`-only, never reversed** — `hr/reports` is the only HR route `MGM` appears on (`HR;MGM` + `manager`, R5, unchanged). This was already correctly built this way at the route/nav/dashboard-card/drill-through-link level (see below) — the actual gap found and fixed was one layer deeper: `get_hr_reports_dashboard_rpc.sql` runs `SECURITY INVOKER`, so its own manager-gated guard only controls who may *call* it — the rows it can return are separately gated by each underlying table's own RLS, and none of the five tables it reads directly (`employees`, `leave_ledger_entries`, `attendance_logs`, `attendance_activities`, `employee_lifecycle_cases`) had any `MGM` policy at all, so an `MGM` manager who passed the guard still got back an empty dashboard. Fixed with `supabase/policies/mgm_hr_reports_access_fix.sql` — deliberately **manager-gated** (`"MGM Manager VIEW"`, not the company-wide `"MGM Department VIEW"` template items 2/4 used above): the only legitimate path into any of these tables is the manager-gated Reports page itself, so a company-wide grant would let a non-manager `MGM` staffer read raw employee/attendance/leave/lifecycle-case rows directly via the REST API — a real over-grant this data's sensitivity doesn't warrant.
...
Sales Reports' "Sales Order Book" tile and "Order Book by Rep" chart both link to `../orders`. Finance Reports has **nine** such links across its "Revenue Invoiced"/"Cash Collected"/"Overdue Risk" tiles and its AR/AP Aging, Unallocated Payments, Top Overdue Vendors, Top Vendors by Spend, and Unallocated Outgoing Payments charts, resolving to `../invoices/list`, `../invoices/payments`, `../bills/list`, and `../bills/vendor-payments` (paths updated 2026-09 for the Invoices/Bills tab merge — see the judgment call #4 note above; the target pages and their `FIN;MGM` gate are unchanged, only the paths nested under the merged tabbed layouts). Without a fix, an `MGM` viewer on either Reports page would click a KPI tile or chart and land on `UnauthorizedUser` — the same bug class this whole matrix was built to catch, just a second instance of it.
...
- `src/pages/user/finance/financialReports/FinancialReports.jsx` computes `canAccessFinanceOps` — one flag covers all nine links, since Invoices/Payments/Bills/Vendor Payments share an identical gate. **Updated 2026-09** for item 4's reversal above: now `canAccess({departments:["FIN","MGM"]})`, was `canAccess({departments:["FIN"]})` — and passed into `getFinanceOverviewConfig(...)` plus every `viewAllTo` prop.
- `src/pages/user/hr/hrReports/HRReports.jsx` computes `canAccessHrOps = canAccess({departments:["HR"]})` — **deliberately excludes `MGM`**, the mirror image of the other bullets above: `hr/employees`/`hr/attendance`/`hr/onboarding`/`hr/offboarding` were never reversed (see item 6 above), so an `MGM` viewer of HR Reports must keep seeing every KPI tile/chart as a plain, non-clickable number, never a link into a page they still can't open. Passed into `overviewConfig.js`'s `getHrReportsOverviewConfig(kpis, canAccessHrOps, filters)`/`getHrReportsNeedsAttentionConfig(...)`, which resolve `employeesTo`/`attendanceTo`/`onboardingTo`/`offboardingTo` the same `? real path : undefined` way. This file already existed correct — nothing about it changed in item 6's fix, which was RLS-only.

**Convention for future pages**: any new drill-through link/button on a Reports page must be gated the same way — computed from `canAccess()` against its *target* route, never assumed to follow just because the viewer can see the page the link lives on, and never assumed to include every department the Reports page itself allows (HR Reports is the standing counter-example: `HR;MGM` can open the page, but the flag gating its own links is `HR`-only, on purpose). `OrdersPageLayout`'s tab bar (`All Orders`/`Budgets`) was checked too — it's currently safe because both tabs share an identical `SAL`+`manager` gate at every level, so no viewer can reach one tab but not the other today, but that stops being true the moment those two gates are ever allowed to diverge.
