...
2. **Sales Clients/Leads/Orders** (`sales/clients`, `sales/leads`, `sales/orders*`) granted `MGM` at the route level even though neither the sidenav nor the dashboard card exposed that — fixed by dropping `MGM` from these routes (R3: Top Management gets the Reports rollup, not raw row-level CRM/order records).
...
- `src/pages/user/sales/reports/Reports.jsx` computes `canAccessOrders = canAccess({departments:["SAL"], roles:["manager"]})` (matching `sales/orders`' actual gate) and passes it into `getSalesReportsOverviewConfig(...)`; the "Order Book by Rep" chart's `viewAllTo` uses the same flag.
